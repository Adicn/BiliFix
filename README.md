...
- 评论AI翻译（实验性功能）
- 网络优化（可选）：使用备用 DNS 解析 B 站相关域名并缓存，并将 B 站自有 DoH 请求从不可用的 Google DNS 降级到可直连的兼容服务，改善首页启动时无法加载的问题

网络优化默认关闭，可在应用内设置的“BiliFix -> 增强 -> 网络优化”中开启。该功能只优化 DNS/DoH 解析；如果当前网络还限制了后续 HTTPS 连接，仍需要使用 VPN。
